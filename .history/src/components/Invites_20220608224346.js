...
function Invites(props) {
    ...
    const handleAccept = (e) => {
        ...
        shows.map((show) => {
            console.log(show)
            if (e.target.id === headliner.id) {
                props.updateHeadliner(true);
            } else if (e.target.id === fourth.id) {
                props.updateFourth(true);
            } else if (e.target.id === third.id) {
                props.updateThird(true);
            } else if (e.target.id === second.id) {
                props.updateSecond(true);
            } else if (e.target.id === first.id) {
                props.updateFirst(true);
            } else if (e.target.id === venue.id) {
                props.updateVenue(true);
            }
        })
        const stateObject = {
            headlinerDecision,
                      ...
                      <tbody>                      
                        {shows && shows.map((show) => {
                            if (
                                ...
                                return (
                                    <tr>
                                        <td id={show.id}><button className="btn btn-warning" onClick={ViewShow} id={show.id}>{show.id} hj</button></td>
                                        <td ><button className="btn btn-warning" id={show.id} onClick={handleAccept}>Accept</button></td>
                                        <td><button className="btn btn-warning" id={show.id} onClick={handleReject}>Reject</button></td>
                                    </tr>
                                ) 
