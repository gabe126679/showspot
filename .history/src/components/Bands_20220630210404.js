            ...
            <div className="profile-border">
                    ...
                    {bands && bands.map((band) => {
                        ...
                        {band.members.map((artist) => {  
                            if (band.activated === true && artist.accepted === true && !acceptedCount.includes(artist.id) && count.length <= band.members.length) {
                                setCount(count + 1);
                                acceptedCount.push(artist.id);
                            } 
                            if (band.members.length === acceptedCount.length && band.activated === false) {
                                props.activateBand(band, true);
                                setStatus("active shows:"); 
                            } else if (band.members.length === acceptedCount.length && band.activated === true) {   
                        ...
                        } else if (band.activated === false) {
                            props.activateBand(band, false);
                            setStatus("pending shows:");
                            return (
                                <tbody >
                                <tr>
                                    <td>{band.bandName}</td>
