...
function Footer(props) {
  ...
  useEffect(() => {
    const imagesRef = ref(storage, 'songs');
    console.log(imagesRef[0]);
    // if (users) {
    //   users.map((user) => {
    //     if (user.firstName === "Tom") {
    //       user.songs.map((song) => {
    //         const storageRef = firebase.storage.ref("songs");
    //         //2.
    //         storageRef.listAll().then(function (res) {
    //             //3.
    //             res.items.forEach((song) => {
    //               song.getDownloadURL().then((url) => {
    //                   //4.
            ...
            <div className="song-container">
              ...
              <div >
                <div className="song-info float-end">
                  <button className="btn btn-primary" onClick={testUser}>song</button>
                  <br/>
                  <br/>
                  <br/>
