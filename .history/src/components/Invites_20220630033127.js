...
function Invites(props) {
    ...
    const BandDecision = async (band, artist, decision) => {
       
        props.updateMember(band, artist, decision);
  
        const acceptedMembers = bands.map((secondBand) => {

            if (secondBand.id === band.id) {
                
                band.members.map((secondMembers) => {
                     
                    if (secondMembers.accepted === true && !accepted.includes(secondMembers.id)) {
                        setAccepted([...accepted, secondMembers.id]);                   
                    }
        ...
        const acceptedAnswer = await acceptedMembers.get()

        console.log(acceptedAnswer);
        
        if (accepted.length === band.members.length) {
            props.activateBand(band, decision);
        } 
