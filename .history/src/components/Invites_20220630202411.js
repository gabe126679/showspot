...
function Invites(props) {
        ...
        shows.map((secondShow) => {
            if (secondShow.id === show.id) {
                show.artists.map((secondArtist) => {
                    if (secondArtist.accepted === true && !accepted.includes(secondArtist.id)) {
                        setAccepted((current) => [...current, secondArtist.id]);;                   
                    }
                })
                if (accepted.length === show.artists.length && show.venueDecision === true) {
    ...
    const BandDecision = async (band, artist, decision) => {
            ...
            if (secondBand.id === band.id) {
                ...
                band.members.map((secondMembers) => {
                    ...
                    if (secondMembers.accepted === true && !memberCount.includes(secondMembers.id)) {
                        setMemberCount((current) => [...current, secondMembers.id]);                   
                    }
        ...
        if (memberCount.length === band.members.length) {
