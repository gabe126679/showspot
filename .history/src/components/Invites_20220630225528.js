                                    ...
                                    if (member.id === auth.uid && member.accepted === true) {
                                        return (
                                            <tr>
                                                ...
                                                <td ><button className="btn btn-warning" id={band.id} onClick={async () => {
                                                    await props.updateMember(band, member, false);
                                                    band.members.map((secondMember) => {
                                                        if (secondMember.accepted === false) {
                                                            setCount(1); 
                                                            console.log(count);
                                                        } 
                                                    })
                                        ...
                                        return (
                                            <tr>
                                                <td id={band.id}><button className="btn btn-warning" id={band.id} onClick={ViewBand}>{band.id}</button></td>
                                                <td id={band.id}><button className="btn btn-warning" id={band.id} onClick={async () => {
                                                    await props.updateMember(band, member, true);
                                                    band.members.map((secondMember) => {
