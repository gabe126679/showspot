        ...
        bands.map((secondBand) => {
            if (secondBand.id === band.id) {
                band.members.map((secondMembers) => {
                    if (secondMembers.accepted === true && !acceptedCount.includes(secondMembers.id)) {
                        acceptedCount.push(secondMembers.id);                   
                    }
                })

            }
        })   
        if (acceptedCount.length -1 === band.members.length) {
            props.activateBand(band, true);
        } else if (acceptedCount.length -1 < band.members.length) {
            props.activateBand(band, false);
        }
        console.log(acceptedCount.length)
    }
