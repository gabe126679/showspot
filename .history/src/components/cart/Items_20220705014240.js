...
const Items = (props) => {
    ...
    const handleIncrease = (e) => {
        ...
        songs.map((song) => {
            if (song.id === e.target.id && total >= 0) {
                const newPrice = song.price.split("$").join("");

                const newTotal = parseInt(newPrice)
                setTotal(songTotal + newTotal);
                song.count += 1;
            }
        })
    }
    ...
    const handleDecrease = (e) => {
        ...
        songs.map((song) => {
            const newPrice = song.price.split("$").join("");
            if (song.id === e.target.id && total >= newPrice) {
    ...
    const handleDelete = (e) => {
        ...
        songs.map((song) => {
            if (song.id === e.target.id) {
                props.updateCart(song.id, song.price);
            }
        })

        const newSongs = songs.filter((song) => {
            if (song.id === e.target.id) {
                const newPrice = song.ticketPrice.split("$").join("");
                const newTotal = parseInt(newPrice)
                setSongTotal(songTotal - newTotal);
                ...
                <div className="container border bg-light">
                    ...
                    <div className="category-tag container center">
                        ...
                        {songs && songs.map((song) => {
                            ...
                            <div className="card black z-depth-0 project-sumarry">
                                <div className="card-content white-text text-darken-3">
                                <span className="card-title">{song.id}</span>
                                <br/>                                <br/>
                            ...
                            <button className="btn btn-two" id={song.id} onClick={handleIncrease}>increase quantity</button>
                            <button className="btn btn-two" id={song.id} onClick={handleDecrease}>decrease quantity</button>
                            <button className="btn btn-three" id={song.id} onClick={handleDelete}>X</button>
                            <p>{song.count}</p>
                            <br/>
                            <br/>
                            <div className="category-tag container text-center">
                    ...
                    <br/>
                    <div className="category-tag container text-center">
                        <h4 className="category-text black-text"> Grand Total: ({clientString} + {clientSong})</h4>
                    </div>
                    <br/>
                </div>
